                    ...
                    <div className="flex flex-col gap-10 lg:flex-row items-center lg:gap-16">
                        <div className="flex flex-col gap-6 flex-1 text-left">
                            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary-light w-fit">
                                <span className="text-primary text-xs font-bold uppercase tracking-wider">{aboutHero.tag}</span>
                            </div>
                            <h1 className="text-text-dark dark:text-white text-4xl lg:text-5xl font-extrabold leading-[1.15] tracking-tight"
                                dangerouslySetInnerHTML={{ __html: aboutHero.title.replace('Comfort & Trust', '<span class="text-primary">Comfort & Trust</span>') }}>
                            </h1>
                            <div className="space-y-4 text-text-muted dark:text-gray-300 text-lg leading-relaxed">
                                {aboutHero.paragraphs.map((p, i) => <p key={i}>{p}</p>)}
                            </div>
                        </div>
                        ...
                        <div className="flex-1 w-full lg:max-w-[500px]">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="h-64 rounded-xl overflow-hidden bg-gray-200">
                                    <div className="w-full h-full bg-cover bg-center" data-alt={experts.images[0].alt} style={{ backgroundImage: `url('${experts.images[0].url}')` }}></div>
                                </div>
                                <div className="h-64 rounded-xl overflow-hidden bg-gray-200 mt-8">
                                    <div className="w-full h-full bg-cover bg-center" data-alt={experts.images[1].alt} style={{ backgroundImage: `url('${experts.images[1].url}')` }}></div>
                                </div>
                            </div>
                        </div>
                        <div className="flex-1 flex flex-col gap-6">
                            <h2 className="text-text-dark dark:text-white text-3xl font-bold tracking-tight">{experts.title}</h2>
                            <p className="text-text-muted dark:text-gray-300 text-lg leading-relaxed"
                                dangerouslySetInnerHTML={{ __html: experts.paragraphs[0].replace('Dr. Afzal Khadir T.', '<strong class="text-text-dark dark:text-white">Dr. Afzal Khadir T.</strong>').replace('Dr. Faraz Afzal T.', '<strong class="text-text-dark dark:text-white">Dr. Faraz Afzal T.</strong>') }}>
                            </p>
                             <p className="text-text-muted dark:text-gray-300 text-lg leading-relaxed">
                            ...
                            <div className="pt-4">
                                <button className="flex items-center gap-2 text-primary font-bold hover:gap-3 transition-all">
                                    Meet Our Team <span className="material-symbols-outlined text-sm">arrow_forward</span>
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            <section className="w-full max-w-[1280px] px-6 lg:px-10 py-16 lg:py-24">
                <div className="flex flex-col gap-12">
                    <div className="text-center max-w-3xl mx-auto flex flex-col gap-4">
                        <h2 className="text-text-dark dark:text-white text-3xl lg:text-4xl font-extrabold tracking-tight">{purpose.title}</h2>
                        <p className="text-text-muted dark:text-gray-400 text-lg">{purpose.paragraph}</p>
                    </div>
                    <div className="grid md:grid-cols-2 gap-6 lg:gap-8">
                        {purpose.cards.map((card, i) => (
                            <div key={i} className="group bg-white dark:bg-gray-800 p-8 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-all">
                                ...
                                <h3 className="text-xl font-bold text-text-dark dark:text-white mb-3">{card.title}</h3>
                                <p className="text-text-muted dark:text-gray-300 leading-relaxed">{card.text}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <section className="w-full bg-primary/5 dark:bg-primary/10 py-20">
                <div className="layout-container flex justify-center px-6">
                    <div className="max-w-[800px] text-center flex flex-col items-center gap-6">
                        <h2 className="text-3xl lg:text-4xl font-bold text-text-dark dark:text-white">{cta.title}</h2>
                        <p className="text-text-muted dark:text-gray-300 text-lg">{cta.paragraph}</p>
